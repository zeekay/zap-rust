...
```rust
zap::generated_code!(mod addressbook_zap);

use addressbook_zap::{address_book, person};
use zap::serialize_packed;

fn write_address_book() -> zap::Result<()> {
...
fn read_address_book() -> zap::Result<()> {
...
All accessors return `zap::Result<T>` for safe handling of malformed input.
