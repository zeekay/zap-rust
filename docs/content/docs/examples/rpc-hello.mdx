...
```rust
zap::generated_code!(mod hello_world_zap);

use zap_rpc::{rpc_twoparty_zap, twoparty, RpcSystem};
use hello_world_zap::hello_world;
    ...
    tokio::task::LocalSet::new()
        .run_until(async move {
            ...
            let hello_world_client: hello_world::Client =
                zap_rpc::new_client(HelloWorldImpl);
...
`zap-rpc` uses `Rc` internally, so tasks must run on a `LocalSet`:
...
```rust
let client: hello_world::Client = rpc_system.bootstrap(rpc_twoparty_zap::Side::Server);
```
...
```rust
async fn say_hello(...) -> Result<(), zap::Error> {
    if name.is_empty() {
        return Err(zap::Error::failed("name cannot be empty".into()));
    }
    // ...
}
```
