---
title: Async I/O
description: Asynchronous message reading and writing with capnp-futures
---

# Async I/O

The `capnp-futures` crate provides async support for reading and writing ZAP messages using the Rust async ecosystem.
...
async fn read_from_socket(stream: &mut TcpStream) -> capnp::Result<()> {
...
async fn read_from_file(path: &str) -> capnp::Result<()> {
...
async fn send_message(stream: &mut TcpStream, name: &str) -> capnp::Result<()> {
...
async fn process_messages<R: AsyncRead + Unpin>(
    ...
    loop {
        match read_message(&mut reader, Default::default()).await {
            ...
            Err(e) if e.kind == capnp::ErrorKind::PrematureEndOfFile => {
                break; // Clean EOF
            }
            Err(e) => return Err(e),
        }
...
async fn handle_connection(stream: tokio::net::TcpStream) -> capnp::Result<()> {
...
`capnp-rpc` uses `Rc` internally, requiring `LocalSet`:
...
async fn read_with_timeout(stream: &mut TcpStream) -> capnp::Result<()> {
    let message = timeout(
        Duration::from_secs(30),
        serialize::read_message(stream, Default::default()),
    )
    .await
    .map_err(|_| capnp::Error::failed("timeout".into()))??;
...
async fn robust_read<R: AsyncRead + Unpin>(reader: &mut R) -> capnp::Result<Option<Message>> {
...
impl MessageSender {
    async fn send(&self, message: &capnp::message::Builder<impl capnp::message::Allocator>) -> capnp::Result<()> {
        let mut buf = Vec::new();
        capnp::serialize::write_message(&mut buf, message)?;
        self.tx.send(buf).await.map_err(|_| {
            capnp::Error::failed("channel closed".into())
        })
    }
}
```
