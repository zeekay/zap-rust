---
title: RPC
description: Capability-based remote procedure calls with capnp-rpc
---

# RPC

ZAP RPC is a capability-based system where object references can be passed over the network. The `capnp-rpc` crate provides a Rust implementation.
...
impl callback::Server for CallbackImpl {
    async fn call(
        ...
        self.sender.send(value).await.map_err(|e| {
            capnp::Error::failed(format!("send failed: {}", e))
        })?;
        Ok(())
    }
}
...
let callback_client: callback::Client =
    capnp_rpc::new_client(CallbackImpl { sender: tx });
...
impl my_service::Server for MyServiceImpl {
    async fn do_something(
        ...
        // Return an error
        Err(capnp::Error::failed("Something went wrong".into()))
    }
}
```
...
```rust
use capnp_rpc::twoparty::VatNetworkOptions;

let options = VatNetworkOptions {
    receive_options: capnp::message::ReaderOptions::new()
