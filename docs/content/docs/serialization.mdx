...
```rust
use zap::message::{Builder, HeapAllocator};
...
```rust
use zap::message::{Builder, SingleSegmentAllocator};

let mut buffer = [zap::Word::NULL; 256];
...
let words: &[zap::Word] = /* aligned buffer */;
...
```rust
let segments: &[&[zap::Word]] = reader.get_segments();
...
// Ensure alignment
let words = zap::Word::slice_from_bytes(&mmap)?;
let reader = serialize::read_message_from_words(words, ReaderOptions::new())?;
